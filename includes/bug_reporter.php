...
<!-- Modal -->
<div id="bug-modal" class="fixed inset-0 bg-black/80 hidden flex-col items-center justify-center p-4 backdrop-blur-sm" style="z-index: 9999;">
  <div class="border border-purple-500/30 rounded-2xl w-full max-w-4xl flex flex-col max-h-[90vh] shadow-2xl overflow-hidden" style="background-color: #0f0f1e;">
    ...
    <!-- Body -->
    <div class="flex-1 overflow-y-auto p-4 sm:p-6 flex flex-col lg:flex-row gap-5 sm:gap-6">
      
      <!-- Form Side -->
      <div class="flex-1 flex flex-col gap-4">
        <div>
          <label class="block text-sm font-medium text-slate-300 mb-1">Description <span class="text-red-400">*</span></label>
          <textarea id="bug-desc" rows="4" class="w-full bg-black/20 border border-white/10 rounded-xl px-4 py-3 text-sm text-white placeholder-slate-500 focus:border-purple-500/50 focus:ring-1 focus:ring-purple-500/50 outline-none transition-all" placeholder="What went wrong? Please be descriptive..."></textarea>
        </div>

        <div>
          <label class="block text-sm font-medium text-slate-300 mb-1">Destination</label>
          <select id="bug-dest" class="w-full bg-black/20 border border-white/10 rounded-xl px-4 py-3 text-sm text-white outline-none">
            <option value="admin">Admin (Local System)</option>
            <option value="github">GitHub Repository</option>
          </select>
        </div>

        <div>
          <label class="block text-sm font-medium text-slate-300 mb-1">Your Email <span class="text-slate-500 font-normal">(optional)</span></label>
          <input type="email" id="bug-email" class="w-full bg-black/20 border border-white/10 rounded-xl px-4 py-3 text-sm text-white placeholder-slate-500 outline-none" placeholder="you@example.com">
        </div>
      </div>
      ...
      <div class="flex-1 flex flex-col gap-2">
        <div class="flex justify-between items-center">
          <label class="block text-sm font-medium text-slate-300">Screenshot <span class="text-slate-500 font-normal">(optional)</span></label>
          <div id="screenshot-actions" class="flex gap-2 hidden">
            <button onclick="clearCanvas()" class="text-xs text-slate-400 hover:text-white px-2 py-1 rounded bg-white/5 hover:bg-white/10 transition-colors">Clear Drawing</button>
            <button onclick="takeScreenshot()" class="text-xs text-purple-400 hover:text-purple-300 px-2 py-1 rounded bg-purple-500/10 hover:bg-purple-500/20 transition-colors">Retake</button>
            <button onclick="removeScreenshot()" class="text-xs text-red-400 hover:text-red-300 px-2 py-1 rounded bg-red-500/10 hover:bg-red-500/20 transition-colors">Remove</button>
          </div>
        </div>
        <div id="canvas-container" class="relative w-full min-h-[200px] sm:min-h-[280px] bg-black/40 border border-white/10 rounded-xl overflow-hidden flex flex-col items-center justify-center">
          
          <!-- Empty State -->
          <div id="screenshot-empty" class="flex flex-col items-center text-center p-6">
            ...
            <button type="button" onclick="takeScreenshot()" class="btn-secondary px-4 py-2 rounded-xl text-sm font-medium transition-colors border border-purple-500/30 text-purple-300 hover:bg-purple-500/10">
              📸 Take a Screenshot
            </button>
          </div>

          <!-- Loading State -->
          <div id="screenshot-loading" class="text-slate-500 text-sm flex-col items-center hidden">
            <svg class="animate-spin h-6 w-6 text-purple-500 mb-2" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg>
            Capturing screen...
          </div>

          <!-- Canvas -->
          <canvas id="bug-canvas" class="cursor-crosshair w-full h-full object-contain hidden" style="touch-action: none;"></canvas>
        </div>
      </div>
    ...
    <div class="px-6 py-4 border-t border-white/10 bg-white/5 flex justify-end gap-3">
      <button onclick="closeBugReporter()" class="px-4 py-2 rounded-xl text-sm font-medium text-slate-400 hover:text-white hover:bg-white/5 transition-colors">Cancel</button>
      <button onclick="submitBugReport()" id="btn-submit-bug" class="px-6 py-2 rounded-xl text-sm font-bold text-white bg-purple-600 hover:bg-purple-500 shadow-lg shadow-purple-900/30 transition-colors flex items-center gap-2">
        Submit Report
      </button>
    </div>
...
async function submitBugReport() {
  ...
  const dest = document.getElementById('bug-dest').value;
